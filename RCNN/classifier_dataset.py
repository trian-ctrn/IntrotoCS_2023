...
class CustomClassifierDataset(Dataset):
    ...
    def __init__(self, root_dir, transform=None):
        ...
        positive_list = []
        negative_list = []
        for idx in range(len(name_id)):
            id = name_id[idx]
            jpg_images.append(cv2.imread(os.path.join(root_dir, 'images', f'IMG_{id}.jpg')))

            positive_annotation_path = os.path.join(root_dir, 'labels', f'IMG_{id}_1.csv')
            ...
            negative_annotation_path = os.path.join(root_dir, 'labels', f'IMG_{id}_0.csv')
...
def test2():
    root_dir = 'classifier_ver13/train'
